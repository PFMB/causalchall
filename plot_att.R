...
att_mean_3 <- round(d[year == "Year 3", mean(ATT)], 2)
att_mean_4 <- round(d[year == "Year 4", mean(ATT)], 2)
...
dat_text <- data.frame(
  ...
  x     = c(-40, -40), # position in plot (50, 50) for non-log results
  y     = c(0.02, 0.02),
  stringsAsFactors = F
)
...
ggsave("logresults_causal_challange.pdf")
