...
analysis <- foreach(i = index, .export=exp.var, .errorhandling="pass") %dopar% {
        ...
        q_w <- rowMeans(cbind(q_w, learner_weights_summary_Q(m_7)))
        g_w <- rowMeans(cbind(g_w, learner_weights_summary_g(m_7)))
        
        cat("Average Q-Weights",q_w,"\n")
        cat("Average g-Weights",g_w,"\n")
        summary(m_7)
        cat("m_7 finished \n")
